...
mkdir -p temp
# create scp files
python create_scp.py -i $input_wav -o temp
...
python convert_trans.py -i $input_trans -o temp/trans.txt \
...
python convert_ctm.py -i temp/out/align.ctm  -w temp/lang/words.txt -o $out_ctm

python convert_ctm.py -i temp/out/phone_alined.ctm  -w temp/lang/phones.txt -o $out_phone_ctm
...
python convert_vtt.py --input_ctm $out_ctm --output_vtt $out_vtt